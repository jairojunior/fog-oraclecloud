    ...
    class Database
      class Real
        def get_access_rule(db_name, rule_name)
          ...
          if rule.nil?
            raise Fog::OracleCloud::Database::NotFound, "Could not find rule (#{rule_name}) attached to #{db_name}"
          end
          response.body = rule
          response
        end
      ...
      class Mock
        def get_access_rule(db_name, rule_name)
          ...
          rule = data[:access_rules][db_name].detect { |r| r['ruleName'] == rule_name }
          ...
          if !rule.nil?
            ...
            raise Fog::OracleCloud::Database::NotFound, "Rule #{rule_name} for #{db_name} does not exist"
          end
        end
      end
    end
