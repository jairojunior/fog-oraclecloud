    ...
    class OracleCloud
      ...
      class Mock
        def create_ip_association(params)
          ...
          instance = data[:instances].detect { |i| i[1]['vcable_id'] = params[:vcable] }
          if instance.nil?
            # TODO: Add error handling. And don't create ip association
          else
            # Update it's networking
            instance[1]['networking']['eth0']['nat'] = (params[:parentpool]).to_s
          end
          response.status = 201
          response.body = data[:ip_associations][name]
          response
        end
      end
    end
